...
import { z } from "zod";
import type { CloudRealtimeClientMsg, CloudRealtimeRole, CloudRealtimeServerMsg } from "@agentorch/shared";
...
export class CloudRealtimeHub {
  ...
  handle(accountId: string, role: CloudRealtimeRole, socket: CloudSocket, msg: CloudRealtimeClientMsg): void {
    ...
    if (role === "web") {
      if (msg.type !== "remote_send") {
        send(socket, {
          type: "remote_error",
          code: "UNSUPPORTED_WEB_MESSAGE",
          message: "Web clients can only send remote_send messages in this beta.",
        });
        return;
      }
      this.forwardRemoteSend(accountId, socket, msg);
      return;
    }
      ...
      socket.on("message", async (raw: Buffer | ArrayBuffer | Buffer[]) => {
        ...
        try {
          ...
          if (
            ...
            (parsed.type === "agent_status" || parsed.type === "agent_message") &&
